...
public class FirmaWindow extends Stage {
    ...
    public FirmaWindow(String title, Stage owner) {
    ...
    private Button btnOpret = new Button("Opret Firma");
    ...
    private void opretDeltager(Event event){
        if (txtNavn != null && txtTlf != null){
            controller.createFirma(txtNavn.getText(), txtTlf.getText());
            txtNavn.clear();
            txtTlf.clear();
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Firma oprettet");
            alert.setHeaderText("Firma oprettet");
            alert.setContentText("Firma oprettet korrekt og sendt til storage");
            alert.showAndWait();
            FirmaWindow.this.hide();
        } else{
